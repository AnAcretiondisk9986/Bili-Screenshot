...
const SYSTEM_SHORTCUTS = [
  // 截图默认 Ctrl+Shift+Q（避开 Edge 内置网页截图的 Ctrl+Shift+S）
  { name: "单张截图", combo: { ctrl: true, shift: true, alt: false, meta: false, code: "KeyQ" } },
...
// 系统级快捷键提示：显示浏览器里实际生效的值（用户可在 chrome://extensions/shortcuts 修改）
async function fillSystemShortcuts() {
  try {
    const cmds = await chrome.commands.getAll();
    const map = {
      "bili-screenshot": "sysScCapture",
      "bili-burst": "sysScBurst",
      "bili-copy": "sysScCopy",
    };
    for (const cmd of cmds) {
      const el = map[cmd.name] && $(map[cmd.name]);
...
$("resetShortcutsBtn").addEventListener("click", () => {
  window._scCapture = { ctrl: true, shift: true, alt: false, meta: false, code: "KeyQ" };
...
async function init() {
  ...
  fillSystemShortcuts(); // 异步填充系统级快捷键实际值，不阻塞表单
  ...
  $("testBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      if (resp && resp.ok) {
        if (resp.fallback) {
          setStatus(`测试图片已保存（子目录创建失败，已改存下载根目录）：下载/${resp.path}`);
        } else {
          setStatus(`测试图片已保存到：下载/${resp.path}`);
        }
      } else if (resp && resp.fallback) {
        setStatus(`测试失败（已尝试降级到下载根目录）：${resp.error || "未知错误"}`, true);
      } else {
